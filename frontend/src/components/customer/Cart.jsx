...
const SHELL =
  'w-full max-w-[min(100%,92rem)] mx-auto px-4 sm:px-6 lg:px-8 xl:px-12'
...
export default function Cart() {
  ...
  const { data: ordersMeta } = useQuery({
    queryKey: ['buyer-orders', 'shop-voucher-eligibility'],
    queryFn: async () => {
      const res = await orderApi.listMine({ page: 1, limit: 1 })
      return res.data
    },
  })

  const hasAnyOrder =
    (ordersMeta?.pagination?.total ?? 0) > 0 ||
    (ordersMeta?.orders?.length ?? 0) > 0
  ...
  useEffect(() => {
    ...
    setPlatformVoucher((prev) => (prev?.newUserOnly ? null : prev))
  }, [hasAnyOrder])
  ...
  const platformVoucherDiscount = useMemo(() => {
    if (!platformVoucher || selectedSubtotal < platformVoucher.minOrder)
      return 0
  ...
  const removeBulkMutation = useMutation({
    ...
    onSuccess: (payload) => {
      syncCartQueryAfterMutation(queryClient, payload)
  ...
  const handleCheckout = () => {
    ...
    navigate('/customer/checkout', {
      state: {
        lineIds: [...selectedIds],
        platformCoupon: platformVoucher
          ? {
              code: platformVoucher.code,
              type: platformVoucher.type,
              discount: platformVoucher.discount,
              minOrder: platformVoucher.minOrder,
              maxDiscount: platformVoucher.maxDiscount,
              title: platformVoucher.title,
            }
          : null,
          ...
          {isLoading ? (
            ...
            <div className="rounded-xl bg-white py-20 text-center shadow-md">
              <img
                src={logoEvo}
                alt=""
                className="mx-auto mb-4 h-16 opacity-40"
              />
